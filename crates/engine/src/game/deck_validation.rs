...
fn quick_commander_check(
    ...
    // not a second inline tally: that helper is what nets out a commander
    // listed in BOTH the command zone and the 99 (CR 903.5a — one physical
...
mod tests {
    ...
    /// deck is still 101 cards. Guards the `saturating_sub(1)` in
    ...
    /// singleton violation. Guards the `saturating_sub(1)` against
