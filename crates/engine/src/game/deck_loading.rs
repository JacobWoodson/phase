...
///
/// CR 201.3 / CR 709.2: grouping compares the RESOLVED face name, never the
...
mod tests {
    ...
    fn resolve_names_groups_mixed_spellings_of_one_card() {
        // CR 201.3 / CR 709.2: a composite name, its glued form, and its
    ...
    fn load_deck_does_not_net_a_commander_slot_in_a_format_without_a_command_zone() {
        // CR 903.1: the command zone (and with it CR 903.5a's "the commander is
        // one of the 100" identity) exists only for the Commander variant. A
    ...
    fn load_deck_with_commanders_creates_command_zone_objects() {
        let mut state = GameState::new_two_player(42);
        // CR 903.1: the command zone exists only for the Commander variant, and
        // the loader places (and nets) the slot only for such a format.
        state.format_config = crate::types::format::FormatConfig::commander();
    ...
    fn resolve_combined_face_commander_name_creates_command_zone_object() {
        ...
        let mut state = GameState::new_two_player(42);
        // CR 903.1: command-zone placement requires a command-zone format.
        state.format_config = crate::types::format::FormatConfig::commander();
        load_deck_into_state(&mut state, &payload);
